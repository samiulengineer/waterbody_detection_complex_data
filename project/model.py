...
class CustomModel(keras.Model):
    ...
    def train_step(self, data):
        ...
        inputs, mask1, mask2 = data

        with tf.GradientTape() as tape:
            y_pred1, y_pred2 = self(inputs, training=True)  # Forward pass
            # Compute the loss value
            # (the loss function is configured in `compile()`)
            loss1 = self.compiled_loss(mask1, y_pred1, regularization_losses=self.losses)
            loss2 = self.compiled_loss(mask2, y_pred2, regularization_losses=self.losses)

        # Compute gradients
        trainable_vars = self.trainable_variables
        gradients = tape.gradient([loss1, loss2], trainable_vars)
        # Update weights
        self.optimizer.apply_gradients(zip(gradients, trainable_vars))
        # Update metrics (includes the metric that tracks the loss)
        self.compiled_metrics.update_state(mask1, y_pred1)
        self.compiled_metrics.update_state(mask2, y_pred2)
